...
- **Phase 2 (anchor-retrieval generation)** — **80% (16/20) deterministic.** A 20-entry canonical @graph table indexed by 12D Geodesic top-1 family prediction is injected as a 17th candidate alongside the 16 wiring votes, with planner+geodesic agreement-gating triggering a +60 score boost. All three failure layers are sidestepped because the entire graph DAG comes from the table — no token-by-token generation is involved.

Phase 2 fixed exactly the 5 prompts the Phase 1b classifier recovered (#1, #2, #3, #6, #17). The 3 regressions (#8, #13, #15) are slot-collisions in the handcoded keyword bag — a learned EKAN encoder that gives each family unique 12D coordinates is predicted to push the headline to 90%+.
