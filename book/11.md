...
- **Phase 2b (20D unique-slot embedder + corrected discount anchor)** — **🎯 100% (20/20) deterministic.** GEO_DIMS bumped from 12 to 20 in `microgpt_geodesic.h` so each held-out family gets a unique axis (eliminates the slot-collisions among `apply_tax`/`savings_rate`/`gross_minus_tax`/`discounted_tax` in slot 5 and `clamped_average`/`distance_midpoint` in slot 9 that caused 3 of 4 Phase 2 failures). The discounted_tax anchor was rewritten to use the native `discount` primitive instead of inverse-direction percentage. The §10.4 prediction "learned EKAN encoder pushes the headline to 90%+" was cashed in *without* a learned encoder — a tightened handcoded keyword bag in unique 20D slots was sufficient.

Phase 2 fixed exactly the 5 prompts the Phase 1b classifier recovered (#1, #2, #3, #6, #17). Phase 2b cleared the remaining 4 (#8, #9, #12, #13, #15 — including the slot-collisions and the discount-anchor bug).
