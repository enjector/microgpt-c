...
#ifdef ENABLE_ROTORQUANT
  rotorquant_init(&g_rq, cfg.n_embd, 3, RQ_MODE_PLANAR, true);
  printf("RotorQuant enabled: 3-bit MSE + 1-bit QJL chunk storage.\n");
#else
  printf("RotorQuant DISABLED. Running baseline MSA (FP32 floats).\n");
#endif
      ...
      if ((step + 1) % 500 == 0 || step == 0) {
        ...
        if (mean_loss < best_loss && step > 0) {
          ...
          snprintf(best_ckpt, sizeof(best_ckpt),
                   "c_vm_codegen_best_step%d_loss%.4f.ckpt", step + 1, mean_loss);
