/*
 * MicroGPT-C — Klotski Simplified Multi-Organelle Demo (Kanban Pipeline)
 * Copyright (c) 2026 Ajay Soni, Enjector Software Ltd. MIT License.
 *
 * Simplified Klotski: 4x5 grid with 2x2 target block + smaller blocks.
 * Goal: slide block A to bottom-centre.
 * Player output: "BD" where B=block ID, D=direction (U/D/L/R).
 * Judge is deterministic (collision check, goal check).
 *
 * Build:
 *   cmake --build build --target klotski_demo
 *   ./build/klotski_demo
 */
...
#define PLANNER_CORPUS "c_klotski_planner.txt"
#define PLAYER_CORPUS "c_klotski_player.txt"
#define PLANNER_CKPT "c_klotski_planner.ckpt"
#define PLAYER_CKPT "c_klotski_player.ckpt"
...
static void apply_move(char *board, char block_id, char direction) {
  ...
  for (int i = 0; i < ncells; i++)
    board[cell(cells[i][0], cells[i][1])] = EMPTY;
  /* Set new */
  for (int i = 0; i < ncells; i++)
    board[cell(cells[i][0] + dr, cells[i][1] + dc)] = block_id;
}

static int is_goal(const char *board) {
  /* A at bottom-centre: rows ROWS-2..ROWS-1, cols 1..2 */
  return board[cell(ROWS - 2, 1)] == 'A' && board[cell(ROWS - 2, 2)] == 'A' &&
         board[cell(ROWS - 1, 1)] == 'A' && board[cell(ROWS - 1, 2)] == 'A';
}
...
static void generate_puzzle(char *board, unsigned int *seed) {
  /* Start from goal, scramble */
  memset(board, EMPTY, BOARD_SIZE);
  /* A at bottom-centre */
  board[cell(ROWS - 2, 1)] = 'A';
  board[cell(ROWS - 2, 2)] = 'A';
  board[cell(ROWS - 1, 1)] = 'A';
  board[cell(ROWS - 1, 2)] = 'A';
  /* Other blocks */
  board[cell(0, 0)] = 'B';
  board[cell(1, 0)] = 'B';
  board[cell(0, 3)] = 'C';
  ...
  /* Random scramble */
  int scrambles = 5 + (int)(rand_r(seed) % 15);
...
int main(void) {
  ...
  microgpt_print_config("MicroGPT-C - Klotski Kanban Pipeline Demo", &g_cfg);
  ...
  printf("Solving %d Klotski puzzles...\n\n", NUM_TEST_PUZZLES);
