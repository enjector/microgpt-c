/*
 * tictactoe_transfer_othello — Transfer Learning Between Organelles (K-12/K-13)
 *
...
int main(int argc, char **argv) {
  ...
  remove("c_tto_ttt.ckpt");
  Organelle *ttt = organelle_train("TTT Source", "c_tictactoe_player.txt",
                                   "c_tto_ttt.ckpt", &cfg, NUM_STEPS);
  if (!ttt) {
  ...
  remove("c_tto_scratch.ckpt");
  Organelle *scratch = organelle_train("Othello Scratch", "c_othello_player.txt",
                                       "c_tto_scratch.ckpt", &cfg, NUM_STEPS);
  if (!scratch) {
  ...
  Docs oth_docs;
  if (opa_load_docs_multiline("c_othello_player.txt", &oth_docs, cfg.max_docs) !=
      0) {
  ...
  remove("c_tto_transfer.ckpt");
  Organelle *xfer_ft =
      organelle_train_transfer("Transfer+FT", "c_othello_player.txt",
                               "c_tto_transfer.ckpt", &cfg, NUM_STEPS, ttt->model);
  if (!xfer_ft) {
  ...
  Docs rnd_docs;
  opa_load_docs_multiline("c_othello_player.txt", &rnd_docs, cfg.max_docs);
  Vocab rnd_vocab;
