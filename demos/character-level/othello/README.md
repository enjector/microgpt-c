...
**Point:** The pipeline wins 67% of Othello games against random with zero strategic encoding — proving that OPA can learn basic positional awareness from 4.5K self-play examples, though parse errors and a 27% loss rate show the limits of character-level board representation for adversarial games.
...
**Proof:** 100 games: 67 wins, 6 draws, 27 losses. Win+draw rate 73%. 1,183 parse errors. Average 31.9 moves per game. Zero planner re-plans. Pipeline time: 69.6s.
...
| Metric | Value |
|--------|-------|
| **Games won (X)** | **67 / 100 (67%)** |
| Games drawn | 6 / 100 (6%) |
| Games lost | 27 / 100 (27%) |
| **Win+Draw rate** | **73%** |
...
1. **67% vs random is solid** — the model has learned positional play beyond random, though a corner-prioritising heuristic achieves ~80-90% on 6×6
2. **1,183 parse errors** — the 36-char board + valid move list is at the limit of what BLOCK_SIZE=128 can handle; longer prompts get truncated
3. **Zero re-plans** — the kanban never needed to reset, suggesting the planner produces consistent (if imperfect) plans
4. **27% loss rate** — losses typically occur when the model misses corner captures or makes moves that give the opponent corner access
...
|------|------|------------|--------------|---------------|
| Mastermind | Deduction | 79% | 25 | Fixed secret, no opponent |
| Sudoku | Constraint | 78% | 184 | Fixed solution |
| **Othello** | **Adversarial** | **67%** | **1,183** | **Reactive opponent** |
| Lights Out | Coupled | 10% | 0 | Deterministic puzzle |
...
```bash
# Generate corpus
cd demos/character-level/othello
