...
**Push:** Scale enriched corpus to 200+ puzzles (30–50% enrichment fraction). Add `trap=1` corpus signal to teach the Mover *when* greedy fails. These are **corpus enrichment** and **orchestrator improvements** — they will raise the system's performance ceiling but do not produce model reasoning.
...
| N_LAYER | 4 |
| MLP_DIM | 384 |
| BLOCK_SIZE | 128 (standard), 192 (combined corpus) |
...
| **v3b (Bare)** | **MD-delta + ensemble+valid** | **460K** | **90% (Bare model)** | **Scaling proved scaffolding redundant** |

### Reasoning Trace Experiment (A/B Test)

The reasoning trace experiment tests whether enriching the training corpus with **pipeline process knowledge** (stall counts, move history, blocked directions) improves performance. See [ORGANELLE_REASONING.md](../../docs/research/RESEARCH_ORGANELLE_REASONING.md) §9.8 for full methodology.
...
**Finding:** Enriched traces cannot *replace* the standard corpus (catastrophic failure: 10%) but can safely *augment* it (no regression: 90%). The current 13% enrichment fraction shows no measurable improvement yet — scaling to 30–50% enriched data (200+ puzzle captures) is needed.

### Phase 5: Scaffolding Removal (Verification)

The 460K model was tested with the **oscillation breaker and replan logic disabled** to see if capacity scaling alone solved the problem.
...
**Conclusion:** Scaffolding is a "capacity bridge." It allows the 64K model to function by catching errors it can't avoid. At 460K, the model has internalized the rules deeply enough that it solves the same puzzles without help.
...
# Standard model (Phase 1+2: traces + enriched corpus)
cmake --build build --target puzzle8_reasoning_demo
./build/puzzle8_reasoning_demo

# Combined model (Phase 4b: standard + enriched corpus, BLOCK_SIZE=192)
cmake --build build --target puzzle8_reasoning_combined_demo
./build/puzzle8_reasoning_combined_demo

# Automated A/B test (runs everything and performs verification)
./demos/character-level/puzzle8_reasoning/run_experiment.sh
```

The experiment script now includes:
1. **Phases 1-4**: Reasoning trace capture and augmentation test.
2. **Phase 5**: Bare model verification (460K).
3. **Phase 6**: Capacity contrast (64K assisted vs bare).

All organelles auto-resume from checkpoints (`puzzle8_*_v3b.ckpt`).

### Output Files

| File | Description |
|---|---|
| `puzzle8_reasoning_traces.txt` | Raw reasoning traces (30 per run) |
| `puzzle8_mover_enriched.txt` | Enriched corpus generated from traces |
| `puzzle8_mover_combined.txt` | Standard + enriched corpus concatenated |
| `reasoning_experiment/` | A/B test results (standard + combined outputs) |

## Corpus Generation

```bash
python3 demos/character-level/puzzle8/generate_corpus.py
```
...
| ~~P1~~ | ~~Reasoning trace experiment~~ | ✅ Done — augmentation safe, no regression |
| **P1** | **Scale enriched corpus** to 200+ puzzles (30–50% enrichment fraction) | May unlock hard-band improvement via process knowledge |
## What Would Reasoning Require?

The 10% failure ceiling (3/30 hard puzzles) is a **corpus gap, not a reasoning gap**. The unsolved puzzles require moves that *contradict the greedy heuristic* — there are simply no training examples for those board positions.

The instinct to add lookahead, BFS, A\*, or RL to the orchestrator to close this gap is correct — but those techniques don't make the organelle reason; they make *the pipeline smarter*. The organelle remains a retrieval engine serving a better search system:

| Technique | Where Intelligence Lives | Is It Reasoning? |
|---|---|---|
| Scale corpus (more BFS examples) | Model weights | No — more retrieval |
| `trap=1` corpus signal | Model weights | No — richer retrieval |
| Orchestrator BFS / A\* | Orchestrator code | No — traditional search |
| Reasoning Trace augmentation | Model weights | No — process retrieval |
| **True reasoning** | **Model weights** | **Yes — first principles** |

True model reasoning at this scale would require the model to derive the answer from axioms (e.g., "tile 7 must pass through position X to reach position Y, which blocks tile 3, so I need to move 3 first"). That kind of sequential logical deduction requires working memory and compositional state tracking that emerge only at billion-parameter scale, or through a fundamentally different architecture (MCTS, symbolic reasoning anchor, or verified-data loops).

**The OPA pipeline IS the reasoning layer.** Each organelle handles one sub-task (detect stall / pick direction / validate move). The *system* reasons through their coordination. This is the intended design — not a workaround.
