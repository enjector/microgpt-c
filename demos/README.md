...
**Proof:** Across 16 experiments covering 11 game domains, real-world signal tasks, and 3 code generation tasks, pipelines consistently elevate weak models. The 8-puzzle pipeline (5 organelles) solves 90% of unseen puzzles, Connect-4 wins 88% with zero invalid moves. Continuous-domain exploration revealed the "Discretisation Wall" — the text wire format's 31-character vocabulary destroys continuous gradients, defining the boundary between tasks where OPA excels and where architectural extensions are needed. The lottery experiment validates the architecture's integrity by hitting an irreducible entropy floor (loss ~0.50) on genuinely unpredictable data.
...
|-----------|---------------|------------|
| [Pentago](character-level/pentago/) | Place + twist adversarial game | **91% win** vs random — best adversarial result |
| [8-Puzzle](character-level/puzzle8/) | 5-organelle pipeline + cycle breaking | **90% solve** (100% easy, 100% med, 70% hard) |
| [Connect-4](character-level/connect4/) | Pipeline rescue + ensemble voting | **88% win** vs random, zero invalid moves |
| [Tic-Tac-Toe](character-level/tictactoe/) | Planner→Player→Judge pipeline | **87% win+draw** vs random |
| [Mastermind](character-level/mastermind/) | Feedback loops + sequential deduction | **79% solve**, 5.3 avg guesses, 0 invalid |
| [Sudoku 4×4](character-level/sudoku/) | Constraint satisfaction | **78% solve** — harder puzzles easier (inverse difficulty) |
| [Othello 6×6](character-level/othello/) | Adversarial positional play | **67% win** vs random |
| [Klotski](character-level/klotski/) | Multi-piece sliding blocks | **62% solve** — fallback-dominated (87% parse errors) |
| [Red Donkey](character-level/reddonkey/) | Asymmetric sliding blocks (2×2 donkey) | **30% solve** — corpus-limited (199 entries) |
| [Hex 7×7](character-level/hex/) | Connectivity-based adversarial strategy | **27% win** — topology uplift from 4% |
| [Lights Out](character-level/lightsout/) | Toggle logic + coupled constraints | **10% solve** — coupled constraints too complex |

### Negative Control

| Experiment | What It Tests | Key Result |
|-----------|---------------|------------|
| [Lottery Prediction](character-level/lottery/) | Independent random events (negative control) | Loss floor **~0.50** — confirms no learnable signal |

### Code Generation

| Experiment | What It Tests | Key Result |
|-----------|---------------|------------|
| [C Code Composition](character-level/c99_compose/) | Planner→Judge pipeline + LR scheduling | **83% exact match**, 98% parse, 1.2M params |
| [VM Code Generation](character-level/vm_codegen/) | Constrained DSL retrieval fidelity | 1,597 functions, 863 tokens, 0% UNK |
| [VM Compose](character-level/vm_compose/) | Multi-candidate best-of-N generation | **15% in-vocab**, **5% OOV** (first ever) |

### The Coordination Funnel

![The pipeline acts as a filter — half the model's moves are illegal but the system still wins 90% of games](../docs/research/OPA.png)
...
|--------|:-:|:-:|
| Markets (cross-asset) | **0.03–0.06** | ⚠️ Low loss was a tautology — features contained the classification rule |
| Lottery (independent) | **0.50–0.61** | ❌ No — irreducible entropy |

The low market loss was a tautology: features contained the classification rule. Permutation tests confirmed that when predicting future regimes, the model cannot beat shuffled labels. Combined with the lottery entropy floor, both experiments serve as **boundary-defining negative results.**
...
|---|---|---|
| Single char (APL-style) | Regime classification (`R`, `I`) | Encoding too sparse for temporal data |
| 4-char code | Mastermind (`ABCD`) | 86% solve, 0 invalid |
| Cell coordinates | Othello (`R3C4`) | 67% win |
...
| Document | Scope |
|----------|-------|
| [RESEARCH_ORGANELLE_PIPELINE](../docs/research/RESEARCH_ORGANELLE_PIPELINE.md) | Wire format design — why pipe-separated flat strings beat free-form C |
| [RESEARCH_ORGANELLE_PLANNER](../docs/research/RESEARCH_ORGANELLE_PLANNER.md) | Kanban coordination protocol — stateful adaptation for stateless models |
| [RESEARCH_INTELLIGENCE](../docs/research/RESEARCH_INTELLIGENCE.md) | Proof of model intelligence — random gap + loss convergence evidence |
| [RESEARCH_ORGANELLE_GAMES](../docs/research/RESEARCH_ORGANELLE_GAMES.md) | Game leaderboard + recommended next puzzles |
| [RESEARCH_WHY_LOGIC_GAMES](../docs/research/RESEARCH_WHY_LOGIC_GAMES.md) | Why games are the experimental apparatus, not the goal |
| [RESEARCH_ORGANELLE_VISION](../docs/research/RESEARCH_ORGANELLE_VISION.md) | CLI tooling design — `microgpt create/train/infer/pipeline` |
| [RESEARCH_GENERALISATION](../docs/research/RESEARCH_GENERALISATION.md) | VM generalisation research — "Composition in the pipeline, not the model" |
| [RESEARCH_ORGANELLE_REASONING](../docs/research/RESEARCH_ORGANELLE_REASONING.md) | Reasoning experiments — retrieval vs true reasoning |
