...
- [x] **Wiring Organelle (Pipeline IR)** — graph-based tool composition (540K params, 368 train examples, **95% strict-verified + 85% executed + 75% correct on all 5 distinct input sets on natural-English transfer** via Phase 13 three-bucket corpus expansion, 88% accuracy among executing graphs, 100% on synthetic templates). Best-of-16 + verify-as-judge + tolerant parser + post-parse `pipeline_repair()` + 40-primitive C-native execution + 20-prompt × 5-input reference suite. See [RESEARCH_PIPELINE_IR](docs/research/RESEARCH_PIPELINE_IR.md).
