...
- [x] **Wiring Organelle (Pipeline IR)** — graph-based tool composition (540K params, **75% strict-verify + 40% executed + 35% numerically correct on natural-English transfer** via Phase 7 reference-answer correctness suite, 87.5% accuracy among executing graphs, 100% on synthetic templates). Best-of-16 + verify-as-judge + tolerant parser + post-parse `pipeline_repair()` + 40-primitive C-native execution + 20-prompt reference suite. See [RESEARCH_PIPELINE_IR](docs/research/RESEARCH_PIPELINE_IR.md).
