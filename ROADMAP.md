...
- [x] **Wiring Organelle (Pipeline IR, multi-organelle, anchor-retrieval)** — graph-based tool composition (540K-param wiring + 540K-param planner + 20-entry anchor table indexed by 20D Geodesic, 368 train examples, **🎯 100% (40/40) deterministic correct on all 5 distinct input sets on natural-English transfer** on the doubled held-out test set. 17-phase + Phase 1a/1b/1c/2/2b/2c arc closed; Phase 2c stress-tests the Phase 2b 100% with 20 lexical paraphrases (e.g. "bmi clipped to a healthy lo hi range" vs "bmi limit it inside lo hi"). 100% holds. 100% on synthetic templates). Best-of-16 + verify-as-judge + tolerant parser + post-parse `pipeline_repair()` + 40-primitive C-native execution + 40-prompt × 5-input reference suite + anchor-retrieval generation. See [RESEARCH_PIPELINE_IR §35–§37](docs/research/RESEARCH_PIPELINE_IR.md) and [RESEARCH_MANIFOLD_LEARNING](docs/research/RESEARCH_MANIFOLD_LEARNING.md).
