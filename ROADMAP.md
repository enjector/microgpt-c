...
- [x] **Wiring Organelle (Pipeline IR, multi-organelle, anchor-retrieval)** — graph-based tool composition (540K-param wiring + 540K-param planner + 20-entry anchor table indexed by 12D Geodesic, 368 train examples, **80% (16/20) deterministic correct on all 5 distinct input sets on natural-English transfer**. 17-phase + Phase 1a/1b/1c/2 arc closed; Phase 2 (anchor-retrieval generation) empirically validates the manifold thesis by replacing autoregressive token generation with table lookup. Remaining 4 failures are slot-collisions in handcoded keyword embedder, addressable by a learned EKAN encoder. 100% on synthetic templates). Best-of-16 + verify-as-judge + tolerant parser + post-parse `pipeline_repair()` + 40-primitive C-native execution + 20-prompt × 5-input reference suite + anchor-retrieval generation. See [RESEARCH_PIPELINE_IR §35](docs/research/RESEARCH_PIPELINE_IR.md) and [RESEARCH_MANIFOLD_LEARNING](docs/research/RESEARCH_MANIFOLD_LEARNING.md).
