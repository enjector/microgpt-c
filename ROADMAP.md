...
- [x] **Wiring Organelle (Pipeline IR, multi-organelle, anchor-retrieval)** — graph-based tool composition (540K-param wiring + 540K-param planner + 20-entry anchor table indexed by 20D Geodesic, 368 train examples, **🎯 100% (20/20) deterministic correct on all 5 distinct input sets on natural-English transfer**. 17-phase + Phase 1a/1b/1c/2/2b arc closed; Phase 2b lifts the Phase 2 80% to 100% by bumping GEO_DIMS 12→20 to give each held-out family a unique slot (eliminates slot-collisions in apply_tax/savings_rate/gross_minus_tax/discounted_tax) and fixing the discounted_tax anchor. 100% on synthetic templates). Best-of-16 + verify-as-judge + tolerant parser + post-parse `pipeline_repair()` + 40-primitive C-native execution + 20-prompt × 5-input reference suite + anchor-retrieval generation. See [RESEARCH_PIPELINE_IR §36](docs/research/RESEARCH_PIPELINE_IR.md) and [RESEARCH_MANIFOLD_LEARNING](docs/research/RESEARCH_MANIFOLD_LEARNING.md).
