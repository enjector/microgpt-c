...
- [x] **Wiring Organelle (Pipeline IR)** — graph-based tool composition (540K params, **75% strict-verify + 40% end-to-end executed on natural-English transfer** to real `w_vm_functions.txt` primitives via Phase 6 native dispatch, 100% on synthetic templates). Best-of-16 + verify-as-judge + tolerant parser + post-parse `pipeline_repair()` + 40-primitive C-native execution. 6/8 arithmetically correct on test inputs. See [RESEARCH_PIPELINE_IR](docs/research/RESEARCH_PIPELINE_IR.md).
