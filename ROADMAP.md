...
- [x] **Wiring Organelle (Pipeline IR)** — graph-based tool composition (540K params, 332 train examples, **80% strict-verify + 50% executed + 35-40% correct on all 5 distinct input sets on natural-English transfer**, 100% on synthetic templates). Phase 11 added 5 structural-diversity template families which broke the topology barrier; primitive-name lexical anchoring is the new bottleneck. Best-of-16 + verify-as-judge + tolerant parser + post-parse `pipeline_repair()` + 40-primitive C-native execution + 20-prompt × 5-input reference suite. See [RESEARCH_PIPELINE_IR](docs/research/RESEARCH_PIPELINE_IR.md).
