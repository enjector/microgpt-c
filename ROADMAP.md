...
- [x] **Wiring Organelle (Pipeline IR)** — graph-based tool composition (540K params, 346 train examples, **100% well-formed + 100% parsed + 75% strict-verified + 55% executed + 50% correct on all 5 distinct input sets on natural-English transfer** via Phase 11 structural diversity + Phase 12 lexical anchoring, 91% accuracy among executing graphs, 100% on synthetic templates). Best-of-16 + verify-as-judge + tolerant parser + post-parse `pipeline_repair()` + 40-primitive C-native execution + 20-prompt × 5-input reference suite. See [RESEARCH_PIPELINE_IR](docs/research/RESEARCH_PIPELINE_IR.md).
