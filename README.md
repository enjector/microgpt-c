...
| 🔌 **Pipeline IR + Wiring Organelle (multi-organelle + manifold-retrieval)** | Typed graph IR (DAG + verifier + text round-trip + DOT) emitted by a 540K-param word-level wiring organelle plus a 540K-param planner organelle. Phase 2 adds **anchor-retrieval generation**: a 20-entry canonical @graph table indexed by 12D Geodesic top-1 family prediction, injected as the 17th candidate alongside 16 wiring votes, with two-classifier (planner + geodesic) agreement gating triggering a +60 score boost. Replaces autoregressive token generation with table lookup for the diffuse-prior prompts that defeated all earlier re-ranking and corpus-engineering levers. | **80% (16/20) deterministic correct on all 5 input sets** on 20 held-out natural-English prompts (Phase 2 closes the manifold thesis empirically; remaining 4 failures are slot-collisions in the handcoded keyword embedder, addressable by a learned EKAN encoder). 100% on synthetic templates. See [the standalone paper](docs/research/RESEARCH_WIRING_ORGANELLE_PAPER.md), the [development log](docs/research/RESEARCH_PIPELINE_IR.md), or the [manifold-learning research](docs/research/RESEARCH_MANIFOLD_LEARNING.md). |
