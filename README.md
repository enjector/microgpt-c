...
| 🔌 **Pipeline IR + Wiring Organelle (multi-organelle)** | Typed graph IR (DAG + verifier + text round-trip + DOT) emitted by a 540K-param word-level wiring organelle plus a 540K-param planner organelle that predicts a graph-name hint to re-rank candidates. Best-of-16 + verify-as-judge + `pipeline_repair()` + 40-primitive C-native dispatch + 5-input reference-answer correctness suite + self-consistency vote re-ranking + planner-family bonus closes the loop **end-to-end with robust verified correctness**. | **80% peak / 75% median correct on all 5 input sets** on 20 held-out natural-English prompts (17-phase arc closed; structural ceiling characterised; future direction: manifold-learning composition). 100% on synthetic templates. See [the standalone paper](docs/research/RESEARCH_WIRING_ORGANELLE_PAPER.md) or the [development log](docs/research/RESEARCH_PIPELINE_IR.md). |
