...
| 🔌 **Pipeline IR + Wiring Organelle (multi-organelle + manifold-retrieval)** | Typed graph IR (DAG + verifier + text round-trip + DOT) emitted by a 540K-param word-level wiring organelle plus a 540K-param planner organelle. Phase 2c ships **anchor-retrieval generation over a 20D Geodesic manifold**: a 20-entry canonical @graph table with each held-out family at a unique axis, indexed by Geodesic top-1 prediction over a handcoded keyword embedder. Anchor injected as the 17th candidate alongside 16 wiring votes; +30 unconditional anchor bonus + planner+geodesic agreement-gated +60 boost. Replaces autoregressive token generation with table lookup. | **🎯 100% (40/40) deterministic correct on all 5 input sets** on the doubled held-out set (20 originals + 20 paraphrases). 100% strict-verified, 100% end-to-end executed, 100% anchor pick-rate. The 17-phase + 6-phase manifold-retrieval arc closes with the ceiling at 100% under lexical paraphrase. See [the standalone paper](docs/research/RESEARCH_WIRING_ORGANELLE_PAPER.md), the [development log](docs/research/RESEARCH_PIPELINE_IR.md), or the [manifold-learning research](docs/research/RESEARCH_MANIFOLD_LEARNING.md). |
