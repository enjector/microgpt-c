...
| 🔌 **Pipeline IR + Wiring Organelle** | Typed graph IR (DAG + verifier + text round-trip + DOT) emitted by a 540K-param word-level organelle trained on 272 (prompt, graph) pairs of real `w_vm_functions` primitives (bmi, compound, sigmoid, gcd, apply_tax …). Best-of-16 + verify-as-judge + `pipeline_repair()` + 40-primitive C-native dispatch + 20-prompt reference-answer correctness suite closes the loop **end-to-end with verified correctness**. | **75% strict-verified** + **40% end-to-end executed** + **35% numerically correct** on 20 held-out natural-English prompts (87.5% accuracy among executing graphs). 100% on synthetic templates. See [RESEARCH_PIPELINE_IR](docs/research/RESEARCH_PIPELINE_IR.md). |
