...
static int apply_move_ttt(char *board, int pos, char player) {
  /* Returns 1 if valid (pos in range and cell empty), 0 otherwise */
  if (pos < 0 || pos > 8)
    return 0;
  if (board[pos] != EMPTY)
    return 0;
...
typedef struct {
  char blocked[64]; /* comma-separated blocked positions */
  char last[64];    /* comma-separated last N positions */
...
static void kanban_add_blocked(Kanban *kb, int pos) {
  char buf[8];
...
static void kanban_add_last(Kanban *kb, int pos) {
  char buf[8];
  snprintf(buf, sizeof(buf), "%d", pos);
...
int main(void) {
  ...
  /* Runtime configuration — same as puzzle8 organelles */
  ...
  for (int game_idx = 0; game_idx < NUM_TEST_GAMES; game_idx++) {
    ...
    int moves_made = 0;
    char result = EMPTY; /* 'X' win, 'O' loss, '_' ongoing */
    int game_draw = 0;
    ...
    while (result == EMPTY && !game_draw) {
      ...
      if (proposed_pos < 0) {
        total_parse_errors++;
        /* Fallback: pick first available empty not in blocked */
        for (int i = 0; i < num_empties; i++) {
          char pos_str[4];
          snprintf(pos_str, sizeof(pos_str), "%d", empties_arr[i]);
          if (strstr(kb.blocked, pos_str) == NULL) {
            proposed_pos = empties_arr[i];
            break;
          }
        }
        if (proposed_pos < 0 && num_empties > 0)
          proposed_pos = empties_arr[0]; /* desperate fallback */
        if (proposed_pos < 0)
          break;
      }
      ...
      if (apply_move_ttt(board, proposed_pos, PLAYER_X)) {
        ...
        kanban_add_last(&kb, proposed_pos);
        kanban_clear_blocked(&kb);
        kb.stalls = 0;
        ...
        kanban_add_blocked(&kb, proposed_pos);
        kb.stalls++;
        total_invalid_moves++;
      }
    }
