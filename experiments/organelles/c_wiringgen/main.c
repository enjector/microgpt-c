/*
 * MicroGPT-C — C Wiring Generation Example (c_wiringgen)
 * Copyright (c) 2026 Ajay Soni, Enjector Software Ltd. MIT License.
 *
 * Trains a character-level GPT on a corpus of function compositions —
 * functions that chain together known primitives (from c_codegen) into
 * higher-level operations.
 *
 * This is the "wiring organelle": it learns HOW to compose functions,
 * while c_codegen learns WHAT each function does.
 *
 * Build:
...
#define CHECKPOINT_FILE "c_wiringgen.ckpt"
#define TRAINING_LOG "c_wiringgen.ckpt.log"
...
int main(void) {
  ...
  /* Runtime configuration — same as c_codegen */
  ...
  microgpt_print_config("MicroGPT-C - C Wiring Generation Demo", &cfg);
  ...
  /* ---- Load wiring corpus (blank-line separated) ---- */
  Docs docs = {0};
  if (load_docs_multiline("c_wiring.txt", &docs, cfg.max_docs) != 0) {
    fprintf(stderr, "Cannot open c_wiring.txt\n");
    return 1;
  }
  shuffle_docs(&docs);
  printf("loaded %zu wiring compositions (multi-line documents)\n",
         docs.num_docs);
  ...
  if (logf) {
    ...
    fprintf(logf, "========================================\n");
    fprintf(logf, "Corpus: %zu compositions | %zu chars (%.1f KB)\n",
      ...
      if ((step + 1) % 500 == 0 || step == 0) {
        ...
        if (mean_loss < best_loss && step > 0) {
          ...
          snprintf(best_ckpt_name, sizeof(best_ckpt_name),
                   "c_wiringgen_best_step%d_loss%.4f.ckpt", step + 1,
                   mean_loss);
...
#define CONFIDENCE_LOW 0.15    /* below = refuse to generate */
  ...
  /* ---- Generate wiring compositions from prompts ---- */
  seed_rng(infer_seed);
  printf("\n--- generated wiring compositions (with confidence) ---\n");
  scalar_t *logits_buf =
  ...
  /* Prompts to test — mix of corpus matches and novel compositions */
  const char *prompts[] = {
      /* ---- NOVEL WIRING (not in corpus) ---- */
      "/* normalize then smooth array */",
      "/* denoise and downsample */",
      "/* bandpass then compute energy */",
      "/* sort then trim and average */",
      "/* ema followed by differentiation */",
      "/* detrend then compute spectrum */",
      "/* rank then compute correlation */",
      "/* filter then compute autocorrelation */",
      "/* smooth two arrays then compute distance */",
  ...
  for (int s = 0; s < num_prompts; s++) {
    ...
    /* If confidence is too low, warn and skip */
    if (prompt_confidence < CONFIDENCE_LOW) {
      printf("  >> prompt not recognized — skipping generation\n");
      continue;
    }

    /* Generate wiring code, tracking per-character confidence */
