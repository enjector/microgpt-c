...
- 3.1 LM Studio health-check + prompt-template iteration log
- 3.2 `tools/llm_game_player.{c,h}` — implementation + smoke test on 20 fixed boards
- 3.3 Distillation corpus generation stats (T6 diagnostics: games played, LLM-X win rate vs random, pairs accumulated, wall-clock)
- 3.4 Training run on distillation corpus (Pathway A or B; loss curve)
- 3.5 Evaluation: 100 games vs random with distilled organelle; per-move latency p99
- 3.6 Comparison table — baseline 88% / OQL 89% / distilled X%
- 3.7 Engine-surface-frozen confirmation (T5, T9: 0-line diff)
