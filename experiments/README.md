...
- E19 ✅ MEASURED — **CLR transfers to games: all 4 targets PASS where E18 found a null.** [E19](E19-game-clr-quality-verifier.md) asks E18's open converse — does CLR work where a *semantic* verifier exists? Built a cheap 1-ply quality verifier for Connect-4 (from the existing `check_winner`). Oracle-first probe: on critical decisions, **Baseline (ensemble) good = 37%, Oracle@16 good = 60% → +23pp headroom** (inverse of E18's wiring null). Then the verifier-gated CLR re-rank + a punishing 1-ply opponent: **T1** critical good-rate 37%→**63%**; **T2** win vs random 91%→94% (masked, as predicted); **T3** win vs punishing O **9%→30% = +21pp** (3.3×, exceeds ≥+10pp 2×); **T4** flag-gated. The ensemble *collapses to 9%* against a punishing opponent (invisible at 91% vs random); CLR re-rank holds 30%. Bounded by the 60% oracle ceiling (can't exceed the generation ceiling). **8-puzzle hard-tier extension (H10, §3.5):** ported with an exact BFS-from-goal distance verifier — **solve-rate NOT lifted** (70% hard, unchanged), because that pipeline already bakes in an MD quality heuristic, so the oracle gap is small (+8pp) and the re-rank only saturates it (marginally shorter solutions, no outcome lift). **Unified finding (the gradient): CLR pays off in proportion to the oracle gap, which is large only when the existing Judge is purely *structural*** — wiring (structural, generation-bound) zero gap → dead (E18); 8-puzzle (MD heuristic present) small gap → saturated, no lift; Connect-4 (legality-only Judge) large gap → big lift. All code gated on `C4_ORACLE`/`C4_RERANK`/`C4_GREEDY_O`/`P8_ORACLE`/`P8_RERANK` (zero default-behaviour change).
- E18 ✅ MEASURED — **CLR falsified as a lift; all 5 targets PASS.** [E18](E18-clr-reliability-reranker.md) maps VibeThinker-3B's Claim-Level Reliability Assessment onto the wiring best-of-16 loop. On the 20 leakage-free clean paraphrases (wiring only): **Oracle@16 = Majority@16 = CLR@16 = 35%** — zero re-ranking headroom (the correct answer is absent from the pool for 13/20 prompts; CLR can't discriminate because the IR verifier is *structural*, so all candidates get reliability r=1 and CLR collapses to majority). Full pool (anchor on): Oracle jumps 35→100% via *retrieval*, but majority/CLR reach only 80% — the residual 20pp is recovered only by the system's *source-trust* bonus, not by claim reliability. Directly formalises the §32/§38/§44 generation-not-ranking thesis with the oracle bound the 17-phase arc never computed. Origin: [`docs/research/papers/VibeThinker/IDEAS.md`](../docs/research/papers/VibeThinker/IDEAS.md) §3. Measurement instrument ships behind `--oracle` (zero default-behaviour change).
- E15 ✅ MEASURED — **T5 = FAIL (thesis falsified at this scale).** Full Phase 4-7 measurement on a follow-up worktree branch. 7 PASS / 1 FAIL. Mono Klotski 64.6% / OPA Klotski 69.9% → +5.3 pp (§1.5 "weakly validated" band). Mono Puzzle15 0.1% / OPA Puzzle15 0.7% → +0.6 pp (§1.5 "thesis falsified at this scale" band). The T5 AND-clause (≥ 15 pp on both tasks) is decisively failed by puzzle15. Per §1.7 this is the most-informative single result the experiment could produce: OPA's value is NOT capacity efficiency at this scale; it IS the deterministic infrastructure surrounding the model. T6 compute equivalence verified: 0.74% / 1.06% imbalance — well within ±10%. T8 engine surface frozen (0 lines diff). Headline reframe of `ORGANELLE_STATE.md` claims is mandatory follow-up. Suggested E16/E17/E18 land the audit/latency/edge / genuine-recovery / multi-task-transfer follow-ons. See [E15 §3.13 + §4](E15-composition-vs-monolithic.md).
