...
- E19 ✅ MEASURED — **CLR transfers to games: all 4 targets PASS where E18 found a null.** [E19](E19-game-clr-quality-verifier.md) asks E18's open converse — does CLR work where a *semantic* verifier exists? Built a cheap 1-ply quality verifier for Connect-4 (from the existing `check_winner`). Oracle-first probe: on critical decisions, **Baseline (ensemble) good = 37%, Oracle@16 good = 60% → +23pp headroom** (inverse of E18's wiring null). Then the verifier-gated CLR re-rank + a punishing 1-ply opponent: **T1** critical good-rate 37%→**63%**; **T2** win vs random 91%→94% (masked, as predicted); **T3** win vs punishing O **9%→30% = +21pp** (3.3×, exceeds ≥+10pp 2×); **T4** flag-gated. The ensemble *collapses to 9%* against a punishing opponent (invisible at 91% vs random); CLR re-rank holds 30%. Bounded by the 60% oracle ceiling (can't exceed the generation ceiling). General lesson (E18+E19): the project's Judges are all *structural* — a cheap *semantic*/quality verifier is the lever that makes test-time scaling pay off, and not before. All code gated on `C4_ORACLE`/`C4_RERANK`/`C4_GREEDY_O` (zero default-behaviour change).
