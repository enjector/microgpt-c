...
- E15 🔬 in-progress on a worktree branch — **Phases 1-3 complete; Phases 4-7 deferred to a follow-up run.** Interim verdict matrix: 4 PASS (T1 OQL grammar, T2 oracle yield 2000/2000=100% on both tasks, T7 zero leakage, T8 engine surface frozen) + 4 DEFERRED (T3-T6 require training-budget that exceeds a single agent-run). Substrate now on disk: deterministic oracles (`puzzle15_a_star`, `klotski_a_star`, both self-test-passing), FROM ORACLE SOURCE clause in OQL, training+held-out corpora (Klotski 2000/113, Puzzle15 2000/948), JSON-line parser + bigram-Jaccard unit tests in CI. Held-out for Klotski under the §1.6 strict-audit discipline is 113 / pre-reg 500 — recorded as T7-driven scope reduction, NOT a relaxed audit threshold. **Section 4 deliberately NOT written** per §1.6: gated on full T3-T6 measurement.
