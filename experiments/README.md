...
- E19 🔬 PROBE MEASURED (full re-rank pre-registered, unrun) — **CLR transfers to games: +23pp headroom where E18 found none.** [E19](E19-game-clr-quality-verifier.md) asks E18's open converse — does CLR work where a *semantic* verifier exists? An oracle-first probe on Connect-4 (cheap 1-ply quality verifier built from the existing `check_winner`): on 273 critical decisions, **Baseline (ensemble pick) good = 37%, Oracle@16 good = 60% → +23pp re-ranking headroom** (the inverse of E18's wiring null). The model generates good moves but the legality-only Judge can't select them. Bounded by a 60% partial generation ceiling; win-rate lift masked by random O (hence the pre-registered §1 re-rank uses a punishing opponent, T3 ≥ +10pp). General lesson: the project's Judges are all *structural* — a cheap *semantic*/quality verifier is the lever that makes test-time scaling pay off. Probe code gated on `C4_ORACLE` (zero default-behaviour change).
