...
3. **Type-directed compositional search** (V1.0.4) over the primitive manifest — a deterministic, beam-width-1 reverse-construction from the desired output type. **First-run achieved baseline: 100 % verified, 30 % correct** on the leakage-audited 30-prompt compositional set (`SLO-WIRE-005`, V1.0.5). Below the pre-registered 50 % target — the gap to the design goal is `GAP-WIRE-006` (`OPEN`), with three pre-registered improvements documented in `RESEARCH_DISCLOSURE.md` §3.5.
