...
| GAP-DEP-001 | strategy | P1 | platform | DEFERRED | — | 2026-04-30 | Phase 1 (fraud) | Drop "pure C99 zero-deps" project policy and adopt the dependency-boundary policy in `docs/DEPENDENCY_POLICY.md` (Categories A/B/C). |
...
| GAP-WIRE-005 | platform | P1 | wiring layer | PARTIALLY-RESOLVED | corpus-author | 2026-04-30 | 2026-05-01 | V1.0.4 ships the mechanism. V1.0.5 measured 9/30 (30 %), V1.0.6 same. **V1.0.7 (Phase 6b) achieves 12/30 (40 %)** via the four-root-cause fix (binder + INPUT_ORDER + manifest-driven prior + earliest-keyword tie-break). Stays `PARTIALLY-RESOLVED` per §5.3 (40–49 % range). New SLO-WIRE-005 baseline = 40 %. The 50 % design target persists; tracked in `GAP-WIRE-006`. See `RESEARCH_DISCLOSURE.md` §5.5 for full per-axis analysis and the residual structural limit (3+ arity outers + keep_dups). |
| GAP-WIRE-006 | research | P1 | wiring compositional search | PARTIALLY-RESOLVED | corpus-author | 2026-05-01 | Phase 6c? | V1.0.7 (Phase 6b) executed the v2.0 fix plan (`COMPOSITIONAL_GENERATOR_FIX_PLAN.md`) — all three streams (D arg-binder + per-port keywords, E `# INPUT_ORDER:` annotation, F manifest-driven prior) plus an earliest-keyword-position tie-break for outer pick. **Achieved 12/30 = 40 %** (axis 1: 2 → 6, axis 2: 4 → 3, axis 3: 3 → 3). Per §5.3 (40–49 % range), `PARTIALLY-RESOLVED` with new SLO-WIRE-005 baseline = 40 %. Hypotheses H4 and H6 confirmed; H5 partially confirmed; H7 not measurably confirmed in isolation. Residual failure mode (3+ arity outers + duplicate inners) motivates a future Phase 6c with structural improvements (limit duplicates, arity-aware outer pick, multi-stage prompt segmentation). Phase 6c not scheduled — opened only on customer signal. |
...
| `docs/STRATEGY_ONE_PAGER.md` | `BRD.md` §5.3 productisation |
| `docs/PRODUCTIZATION_VERTICALS.md` | `BRD.md` §5.3 |
| `docs/DEPENDENCY_POLICY.md` | `BRD.md` §5.4, `GAP-DEP-001` |
