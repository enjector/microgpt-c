...
| GAP-WIRE-005 | platform | P1 | wiring layer | PARTIALLY-RESOLVED | corpus-author | 2026-04-30 | 2026-05-01 | V1.0.4 ships the mechanism (Stream B of compositional fix). V1.0.5 records the honest first-run outcome on the 30-prompt leakage-audited held-out: **30/30 verified (100 %)**, **9/30 correct (30 %)** — below the §3.2 pre-registered target of 50 %. Per §3.3 skip rule (Axis 1 + Axis 2 = 6/20 = 30 %, above the 5/20 = 25 % floor), this is `PARTIALLY-RESOLVED`, not silently re-tuned. The 30 % score is the new SLO baseline. The follow-up is tracked in new `GAP-WIRE-006`. See `RESEARCH_DISCLOSURE.md` §3.5 for failure-mode analysis. |
| GAP-WIRE-006 | research | P1 | wiring compositional search | OPEN | — | 2026-05-01 | Phase 6b? | V1.0.5 baseline 30 %. V1.0.6 (Phase 6) implemented all three pre-registered improvements (beam=2, keep-dups, geo-tie-break) and achieved **same 30 % (9/30)** — below the §4.3 < 40 % failure threshold. Per the §4.3 disposition rule, this **falsifies the simple-search hypothesis**: the dominant failure mode is not greedy outer picking but argument-to-port routing under multiple inner candidates. Phase 6b (learned ranker / external embeddings) remains DEFERRED indefinitely; gap stays OPEN with the V1.0.5 30 % baseline. The Phase 6 changes are retained in source — they exposed a structural duplicate-inner misrouting failure mode that is informative for the next iteration. See `RESEARCH_DISCLOSURE.md` §4.5 for full per-axis comparison and per-prompt sign analysis. |
