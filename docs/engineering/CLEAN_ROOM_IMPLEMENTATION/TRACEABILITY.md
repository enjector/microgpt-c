...
| GAP-WIRE-005 | platform | P1 | wiring layer | PARTIALLY-RESOLVED | corpus-author | 2026-04-30 | 2026-05-01 | V1.0.4 ships the mechanism. V1.0.5/V1.0.6 = 30 %. V1.0.7 (Phase 6b) = 40 %. V1.0.8 (Phase 6c) = 50 % (re-run with H8 OFF actually shows 16/30 = 53 % — V1.0.8 quote was 1 prompt low). **V1.1.0 (Phase 6d, H8 only) = 19/30 (63 %)** — pre-registered §8 PARTIALLY-RESOLVED band (60–69 %). New SLO-WIRE-005 baseline = 63 %. H8 mechanism: per-port noun-aware inner picker + dedup fallback (when noun-affinity doesn't disambiguate) + symmetry-aware override (preserve duplicates when inner keyword appears twice in prompt) + single-letter variable filter. H9 (depth-2 recursion) not implemented; remaining failures motivate a future Phase 6e on the **binder's positional scoping** rather than further inner-picker tuning. See `RESEARCH_DISCLOSURE.md` §8.7. |
| GAP-WIRE-006 | research | P1 | wiring compositional search | PARTIALLY-RESOLVED | corpus-author | 2026-05-01 | Phase 6e? | V1.0.8 (Phase 6c) executed the `COMPOSITIONAL_GENERATOR_FIX_PLAN.md` v2.0 mining-sketch streams G1 (per-primitive synonym lift), G2 (after-connective re-ordering), G3 (binder audit), G4 (disabled the legacy `wiring_geo` substring bump). V1.1.0 (Phase 6d) executed the H8 hypothesis from `COMPOSITIONAL_GENERATOR_PHASE_6D_PLAN.md`. **V1.1.0 = 19/30 = 63 %** (Δ +3 prompts over V1.0.9 16/30). Per §8.4 (60–69 % range), `PARTIALLY-RESOLVED` with new SLO-WIRE-005 baseline = 63 %. Pre-registered §8.3 70 % aggregate target NOT MET — H8 hit +3 of the predicted +6, H9 not implemented. The 11 residual failures are mostly **binder positional-scoping** issues (correct primitive set, wrong wiring) with a minority needing depth-2 (H9 territory). Phase 6e (binder positional scoping) is the highest-leverage residual fix; not scheduled. |
...
| GAP-WIRE-009 | research | P2 | wiring compositional search | PARTIALLY-RESOLVED | corpus-author | 2026-05-01 | 2026-05-01 | V1.1.0 (Phase 6d) executed the H8 hypothesis: per-port noun-aware inner picker. **Result: 19/30 = 63 %** (Δ +3 over V1.0.9). Per §8.4 (60–69 %), PARTIALLY-RESOLVED. The pre-registered 70 % aggregate target was NOT met; H8 captured +3 of the predicted +6. H9 (depth-2 inner recursion) not implemented — residual analysis shows the highest-leverage remaining fix is the **binder's positional scoping** (~5–6 of the 11 residual failures have correct primitive sets but wrong wiring). H9 alone was predicted to add only +2/3, not worth the architectural cost. New `GAP-WIRE-010` opens for the binder positional scoping work; this gap closes at PARTIALLY-RESOLVED. |
| GAP-WIRE-010 | research | P3 | wiring compositional search — binder positional scoping | OPEN | — | 2026-05-01 | Phase 6e (customer-signal-gated) | Identified during V1.1.0 (Phase 6d) outcome analysis. ~5–6 of 11 residual failures on the compositional held-out have **correct primitive sets** but **wrong wiring** (e.g. prompt 25 `sigmoid of x − 3y` produces `[sigmoid, triple_val, subtract]` — right primitives — but match=0/5 means the binder routed `triple_val` against the wrong subtract input). The fix is in `wiring_arg_binder.c`: when picking which prompt-noun feeds which port, weight by the noun's positional proximity to the inner's keyword. Currently the binder uses repeated-noun unification + outer-port-keyword inheritance, which is order-insensitive within a port. Adding a positional-scoping prior (the inner's keyword position should fall between the assigned noun's position and the next noun's position) is predicted to lift +4/11 = ~+13 pp. Not scheduled — opens only on customer signal that ≥ 70 % is required. |
| GAP-OPA-001 | feature | P2 | OPA scaffolding — ACT halting | PARTIALLY-RESOLVED | corpus-author | 2026-05-01 | 2026-05-01 | Phase 7 / V1.2.0. **Primitive shipped:** `OpaActHalting` struct + `opa_act_init` / `opa_act_init_custom` / `opa_act_observe` (survival-style accumulation: `cum += p_local * (1 - cum)`) / `opa_act_should_halt` / `opa_act_below_floor` API in `microgpt_organelle.{h,c}`, plus 4 unit tests (init, accumulation, threshold-cross, below-floor) — all pass. **Demo integration NOT shipped** — opens with customer signal per `OPA_ADAPTIVE_DEPTH_ROADMAP.md` § "Trigger criteria". Hypothesis H10 (8-puzzle hard-tier 30 %→80 %) remains FALSIFIABLE but NOT YET MEASURED. The pre-registered §9.7 outcome stays empty until measurement lands. |
| GAP-OPA-002 | feature | P2 | OPA scaffolding — frozen-input injection | PARTIALLY-RESOLVED | corpus-author | 2026-05-01 | 2026-05-01 | Phase 7 / V1.2.0. **Primitive shipped:** `OpaFrozenInput` struct + `opa_freeze_input` / `opa_prefix_with_frozen` (idempotent — calling twice doesn't double-prefix) API in `microgpt_organelle.{h,c}`, plus 2 unit tests (freeze-basic, prefix-idempotent) — all pass. **Demo integration NOT shipped** — opens with customer signal per the roadmap. Hypothesis H11 (Connect-4 cycle-detector trips −30 %) remains FALSIFIABLE but NOT YET MEASURED. |
| GAP-OPA-003 | feature | P3 | OPA scaffolding — loop-index step token | OPEN | — | 2026-05-01 | Phase 7b | Per `OPA_ADAPTIVE_DEPTH_ROADMAP.md` Mechanism 3. Requires either (a) a re-trained planner organelle that has seen `STEP\|t=N\|` prefixes, or (b) a KL-divergence rig over the existing planner's next-token softmax. Not scheduled in V1.2.0 — opens with customer signal. Hypothesis H12 (KL ≥ 0.1 on ≥ 30 % of replan transitions) FALSIFIABLE but NOT MEASURED. |
| GAP-OPA-004 | research | P3 | OPA scaffolding — depth-extrapolation measurement | OPEN | — | 2026-05-01 | Phase 7b | Per `OPA_ADAPTIVE_DEPTH_ROADMAP.md` Mechanism 4. Requires 8×8 Connect-4 board scaffolding + reference opponent. Pure measurement (no falsification floor). Not scheduled in V1.2.0 — heavy-lift demo work. Hypothesis H13 (8×8 win rate ≥ 40 % at depth=2N from 7×6-trained 460K organelle) is an open question, not a SLO. |
...
| 1.2.0 | 2026-05-01 | Phase 7 primitives — OPA Adaptive-Depth roadmap (`OPA_ADAPTIVE_DEPTH_ROADMAP.md` v1.0) executed at the API layer per `RESEARCH_DISCLOSURE.md` §9. **M1 (GAP-OPA-001)** ships `OpaActHalting` struct + `opa_act_init` / `opa_act_init_custom` / `opa_act_observe` (survival-style accumulation `cum += p * (1 - cum)`) / `opa_act_should_halt` / `opa_act_below_floor` in `src/microgpt_organelle.{h,c}`, with 4 unit tests (init, accumulation, threshold-cross, below-floor). **M2 (GAP-OPA-002)** ships `OpaFrozenInput` struct + `opa_freeze_input` / `opa_prefix_with_frozen` (idempotent) helpers, with 2 unit tests (freeze-basic, prefix-idempotent). All 6 new tests + the existing 59 tests pass — `test_microgpt_organelle` reports 65/65. **M3 (GAP-OPA-003)** and **M4 (GAP-OPA-004)** explicitly NOT shipped in V1.2.0 — M3 needs a KL-divergence rig over the planner softmax (or planner retraining), M4 needs 8×8 Connect-4 board scaffolding. Both are Phase 7b scope, customer-signal-gated. Demo integration of M1/M2 also NOT shipped — primitives are inert until a customer-funded evaluation run opts in. This preserves `BS_organelle.md` §1, §2 invariants bit-identically and the V1.0 demo numbers. ctest 15/15 still green. New gaps `GAP-OPA-001..004` opened. Pre-registered §9.7 outcome stays empty until measurement lands — per the methodology, the V1.0 baseline is the published number. |
| 1.1.0 | 2026-05-01 | Phase 6d implementation pass per `COMPOSITIONAL_GENERATOR_PHASE_6D_PLAN.md` and pre-registered `RESEARCH_DISCLOSURE.md` §8. **H8** (per-port noun-aware inner picker) implemented in `wiring_compositional_search.c::discover_inner_picks_v2`: the picker tokenises the prompt, identifies the expected noun for each outer input port via `outer->port_keywords[ip]`, picks an inner candidate whose own keywords/port_keywords accept that noun, and consumes the noun so subsequent ports see a different budget. Plus a **dedup fallback** that mirrors V1.0.9's `WIRING_KEEP_DUPS=0` post-pass when noun-affinity does NOT disambiguate (most outers like `subtract`/`add`/`min_two` have generic `a`/`b` port_keywords). Plus a **symmetry-aware** override (preserve duplicates when the inner's keyword appears twice in the prompt — "gcd of x squared and y squared"). Plus a **single-letter variable filter** (port_keywords like `x`/`y`/`n`/`r` don't suppress dedup). H9 (depth-2 inner recursion) NOT IMPLEMENTED — predicted +2/3 lift not worth the architectural cost given residual analysis. Compile-time toggles: `WIRING_PORT_AWARE_INNER` (default ON), `WIRING_INNER_DEPTH=2` (default), `WIRING_NOUN_COVERAGE_BONUS` (default OFF — measured net-zero). **Result: 30/30 verified, 19/30 correct (63 %)** — Δ +3 prompts over V1.0.9 16/30 (53 %). Per §8.4 (60–69 % band), `GAP-WIRE-005`, `GAP-WIRE-006`, `GAP-WIRE-009` stay `PARTIALLY-RESOLVED` with new SLO-WIRE-005 baseline = 63 %. Pre-registered §8.3 70 % aggregate target NOT met (H8 captured +3 of predicted +6); H9 falsified by skip rather than measurement. New `GAP-WIRE-010` opens for the binder positional-scoping work (highest-leverage residual fix, ~5–6 of 11 remaining failures have correct primitive set but wrong wiring). Existing wiring numbers preserved (anchor 100 %, fragment 60 %, transformer 35 %, TF-IDF 90 %); ctest 15/15 still green. Leakage audit unchanged (held-out file untouched). |
