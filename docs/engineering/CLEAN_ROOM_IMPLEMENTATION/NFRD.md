...
| SLO-WIRE-005 | Phase 5 compositional held-out. Pre-registered §3.2 target was ≥ 50 % (15/30). **V1.0.8 (Phase 6c) achieves 50 % (15/30) verified-and-correct, 100 % (30/30) verified** — the original design goal met (V1.0.5 30 % → V1.0.7 40 % → V1.0.8 50 %, via per-primitive synonym lift, gated after-connective re-ordering, and disabling the net-harmful legacy geo substring bump). 50 % is the new SLO baseline; the §6.3 60 % full-RESOLUTION gate persists in `GAP-WIRE-006`. | `RESEARCH_DISCLOSURE.md` §6.5 |
