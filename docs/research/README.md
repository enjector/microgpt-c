...
| Evaluating cross-architecture borrowings (e.g. OpenMythos / RDT) | `RESEARCH_OPENMYTHOS_CROSS_POLLINATION.md` — four pre-registered experiments (A/B/C/D) + six rejected techniques with reasons |
| Planning the next research window | `RESEARCH_OPA_DIRECTIONS.md` — sixteen pre-registered OPA experiments across seven directions, with prioritisation matrix and explicit non-pursuit list |
