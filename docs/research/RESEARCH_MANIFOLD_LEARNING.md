...
8. ✅ Phase 2d: leakage audit. 13 of 20 originals verbatim in training (Phase 13 leak). Restated: anchor-retrieval mechanism 🎯 100% (20/20) on novel paraphrases; wiring transformer alone 35% (7/20). [§38]
9. ✅ **Phase 3a: TF-IDF learned encoder falsified.** Pre-registered §40 predicted 12-16/20 on adversarial axis-2; actual 4/20. Per §40.7 skip condition, Phase 3a-full (EKAN-Network) and Phase 3c (RAG fallback) cancelled. The handcoded keyword bag is the right component for the 408-example regime; corpus expansion (Phase 4) is the right corrective if scaling. [§40, §41]
10. ✅ Phase 3b: fragment composition. 6/10 multi-stage prompts correct (within pre-registered 5-7/10 target). 20/20 no-regression on Phase 2c clean. Fidelity-trumps gate makes the composition candidate dominate when its primitive set matches the prompt's expected primitives and no anchor does. [§42, §43]
11. ✅ Arc closed at the architecture level. The remaining frontier is engineering — three of four boundary axes (novel families, weak keyword overlap, domain-vocabulary drift) are open and corpus-bounded; axis 3 (multi-stage compositions) is closed by Phase 3b at 60%. [§44]
12. ✅ **Phase 4 corpus expansion shipped: 4,102-prompt expanded corpus + TF-IDF centroid classifier hits 18/20 (90%) on adversarial axis-2 (vs pre-registered 8-12/20) and 20/20 no-regression on Phase 2c clean. EKAN follow-up cancelled per pre-registered §45 outcome logic — simpler model exceeded the escalation trigger. Axes 2 and 4 "soft-closed" within the curator's synonym range.** [§45, §46]
