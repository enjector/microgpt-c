...
*MicroGPT-C — research paper, April 2026 (v3.5, post-Phase-4: anchor-retrieval 🎯 100% on novel paraphrases, fragment composition 6/10 on multi-stage, **TF-IDF over Phase 4 expanded corpus 18/20 on adversarial axis-2**, wiring transformer documented at 35% true generalisation; remaining frontier is engineering, not research; see development log §44–§46)*
...
**Phase 3 pre-registered predictions (§40)** and **Phase 3a falsification (§41)**: a TF-IDF centroid classifier — the simplest learned encoder — scored 4/20 on the pre-registered adversarial axis-2 target of 12-16/20, and ~8/20 on the no-regression target of ≥18/20. Per the pre-registered §40.7 skip condition, Phase 3a-full (EKAN-Network classifier) and Phase 3c (RAG fallback) are cancelled. The handcoded keyword bag is the right encoder for the 408-example regime, full stop.

**Phase 3b (fragment composition, §42 + §43) shipped:** 6/10 on a 10-prompt multi-stage composition test set, within the pre-registered 5-7/10 target, with 20/20 no-regression on Phase 2c clean paraphrases. A 15-fragment table + composition operator chains 2-3 fragments by output→input linkage; a fidelity-trumps gate (`score += 1000` when composition has the expected primitive set and no anchor does) makes composition dominate on multi-stage prompts without disturbing the single-anchor 100% headline.

**Architectural state of the arc (§44):** the 17-phase corpus-engineering arc + 8-phase manifold-retrieval addendum (1a/1b/1c/2/2b/2c/2d/3a/3b) collectively close the research questions the architecture can pose. Three of four boundary axes (novel families, weak keyword overlap, domain-vocabulary drift) remain open but share a single root cause — the curated library is bounded by hand effort.

**Phase 4 corpus expansion (§45 + §46) shipped:** `tools/corpus_expand.c` generates a 4,102-prompt expanded corpus from per-family synonym tables × sentence templates; the TF-IDF centroid classifier trained on this corpus achieves **🎯 18/20 (90%) on the adversarial axis-2 stress test** (vs pre-registered 8-12/20) and **20/20 no-regression on Phase 2c clean paraphrases**. The pre-registered §45.2 outcome logic cancelled the EKAN-Network follow-up (4b-full) because TF-IDF exceeded the escalation trigger window. The lexical-robustness frontier moved outward: axes 2 and 4 of the four-axis boundary are now "soft-closed" within the curator's synonym range. Remaining engineering scope: anchor-table extension (axis 1) and synonym-table extension (axes 2/4 outside current coverage).

The discipline-of-pre-registration result: §40, §42, §45 were committed *before* results — dictating the response unambiguously when 3a missed (cancel 3a-full and 3c), when 3b's first iteration scored 2/10 (force the fidelity-trumps gate), and when 4b-lite hit 18/20 (cancel 4b-full because the simpler model exceeded the trigger).
