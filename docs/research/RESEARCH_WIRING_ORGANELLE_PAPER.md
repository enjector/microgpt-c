...
*MicroGPT-C — research paper, April 2026 (v3.4, research arc closed: anchor-retrieval 🎯 100% on novel paraphrases, fragment composition 6/10 on multi-stage; wiring transformer documented at 35% true generalisation; remaining frontier is engineering, not research; see development log §44)*
...
**Architectural state of the arc (§44):** the 17-phase corpus-engineering arc + 8-phase manifold-retrieval addendum (1a/1b/1c/2/2b/2c/2d/3a/3b) collectively close the research questions the architecture can pose. Three of four boundary axes (novel families, weak keyword overlap, domain-vocabulary drift) remain open but share a single root cause — the curated library is bounded by hand effort. The remaining frontier is *engineering scope* (curate more anchors/fragments/keywords; or commit to Phase 4 corpus expansion + learned encoder), not *research*. The discipline-of-pre-registration result: §40, §42 were committed *before* results, dictating the response unambiguously when 3a missed and again when 3b's first iteration scored 2/10 (forcing the fidelity-trumps gate to bring it to 6/10).
