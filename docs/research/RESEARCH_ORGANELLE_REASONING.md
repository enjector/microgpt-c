...
The following questions are directly motivated by the experimental results documented in this file. They are ordered by the quality of the evidence already behind them.

---

### Q1 — Can reasoning traces unlock behaviour that looks like reasoning?

**Evidence:** Phase 5 showed that the 460K bare model hits 90% — the same ceiling as the assisted model. The 3 unsolved hard puzzles require moves that contradict the greedy heuristic and have no training examples. The enriched corpus experiment (255 trace-enriched docs) showed no regression but also no improvement at 90% — the corpus was too small (data-scarcity bottleneck, not a model-capacity bottleneck).

**Question:** If the enriched corpus were scaled to 2,000+ puzzles with process traces (including stall recoveries, rejection histories, and non-greedy detour examples), would the model learn *when* greedy fails — effectively internalising pipeline-level decision logic?

This is the **Reasoning Traces as Training Data** hypothesis. It is analogous to Chain-of-Thought prompting for large models: the trace provides the scratchpad structure that the model's weights can then approximate. The key difference is that here, the traces are generated by a *known-correct* deterministic pipeline, so the training signal is noise-free.

**Connection to NAR:** The CLRS-30 enriched-trace experiments show that models trained on *execution traces* (step-by-step algorithm outputs) generalise significantly better than models trained only on input-output pairs. This is the same hypothesis applied at sub-1M parameter scale.

**Experiment:** Scale enriched corpus to 2,000 puzzles (30–50% enrichment fraction), re-run Phase 4 A/B. If the enriched-only model breaks the 90% ceiling, this is evidence that process retrieval can supplement pattern retrieval in a meaningful way.

---

### Q2 — Is the logic operator class the binding constraint for hard games?

**Evidence:** Sudoku (78% solve), Lights Out (10% solve), and Hex (4% win) are the three lowest-performing games. All three have one thing in common: winning requires reasoning over *multiple simultaneous constraints* — exactly what the NAR taxonomy calls **logic operators** (AND/OR/NOT reasoning over multiple propositions at once). The games that perform well (Tic-Tac-Toe 87%, Connect-4 88%, Pentago 91%) are dominated by *local pattern matching*, which organelles retrieve well.

**Question:** Is the performance gap in Sudoku, Lights Out, and Hex caused by the model failing to internalise logic operators — and would a deterministic constraint solver as an external organelle (rather than trained into the model) close the gap?

**Proposed experiment:** Add a constraint-propagation organelle (deterministic, ~50 lines of C) to the Sudoku pipeline that filters the Mover's output to only valid moves given current row/column/box constraints. This externalises the logic operator class exactly as `OpaKanban` externalises state-tracking. Measure the solve rate change.

**Prediction:** If the constraint solver closes the gap significantly, this confirms that logic operators are a distinct capability class that cannot be efficiently trained into sub-1M parameter models — the NAR thesis at this scale.

---

### Q3 — Are enriched corpus experiments comparable to CLRS-30?

**Evidence:** The enriched corpus experiment (`trap=1` detour signal, reasoning traces, rejection histories) is structurally identical to the CLRS-30 research programme: both test whether neural models trained on *algorithm execution traces* can learn to generalise the algorithm, rather than just memorise input-output mappings.

**Question:** Do the MicroGPT-C results at 30K–460K parameters replicate the CLRS-30 scalability barrier — and does the OPA architecture (externalising search to the orchestrator) outperform pure scaling in exactly the regime where CLRS-30 models fail?

**Significance:** CLRS-30 experiments run at 1M+ parameters and still show generalisation failure. If OPA achieves competitive or better performance at 460K params by externalising search rather than trying to learn it, this would be a novel result: *deterministic orchestration substitutes for scale in the NAR generalisation regime*.

**Suggested write-up:** The puzzle8 experiments (Phases 1–5) could be directly reframed as a CLRS-30-style evaluation and compared to published baselines. The OPA architecture's approach would be the experimental variable.

---

### Q4 — What is the minimum deterministic scaffold for any new game domain?

**Evidence:** Across 11 games, the pattern is clear: the OPA pipeline transforms 50–55%-accurate models into 87–91%-accurate systems. The 340-line coordination library (Kanban + cycle detection + judge) is the consistent enabler. But the implementation details vary significantly per game.

**Question:** Can the four NAR neural operator classes (logic, status/comparison, search, state-tracking) serve as a *design checklist* for any new game domain — telling a developer exactly which pipeline components to build before training any models?

**Proposed framework:**
1. Identify which of the four operator classes the game requires
2. Implement deterministic organelles for each required class (not trained — hard-coded)
3. Train neural organelles only for the fuzzy/pattern-matching sub-tasks
4. Measure: can a ~460K model match or exceed the OPA baseline without the deterministic components?

**Significance:** If this checklist reliably predicts which pipeline components are needed and which can be handled by the neural model, it becomes a principled architectural methodology — not just an empirical observation.
...
| **2. Compositional Retrieval** | Pipeline assembles known pieces into novel combinations | `c99_compose`: 65% judge-pass via Planner→Worker→Judge | 50–91% depending on coordination | Retrieval + deterministic wiring |
